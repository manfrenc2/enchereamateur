...
$(document).ready(function() { // Fonction s'execute page chargé
		...
		var table2 = $('#nowbid').dataTable({ // Récupération de l'id de table membre pour modifier le tableau
	...
	$('.crud-delnowbid').click(function(event) {
		...
		var ida = row.find(':first').find(':first').serialize()+"&action=delnowbid"; // on trouve l'ida correspondant à la ligne, et on utilise row pour éviter à jQuery de reparcourir le DOM
		console.log("ida: " + ida);
